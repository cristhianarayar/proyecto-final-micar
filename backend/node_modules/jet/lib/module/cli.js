...
const JetBeforeHook = z.function().args(JetConfigSchema).returns(z.union([z.promise(JetConfigSchema), JetConfigSchema]));
const JetAfterHook = z.function().args(JetConfigSchema).returns(z.union([z.promise(z.undefined()), z.undefined()])).optional();
...
yargs(hideBin(process.argv)).scriptName('jet').version('version', 'Show version number & exit', jetPackageJson.version).alias('v', 'version').option('target', {
  ...
  if (!target) {
    console.log(`[🟥] Target "${argv.target}" not found in your Jet config.`);
    process.exit(1);
  }
  ...
  if (!(await isMetroRunning(argv.metroPort))) {
    console.warn(`[🟨] Metro is not running (${argv.metroPort} via '--metro-port' flag.). Start it before tests to enable stack trace symbolication.`);
  }
  console.log('[🪝] Running before hook...');
  const beforeHookReturnedConfig = await target.before?.(mergedConfig);
  if (!beforeHookReturnedConfig) {
    console.log(`[🟥] Before hook on target "${argv.target}" must return a config object.`);
    process.exit(1);
  }
